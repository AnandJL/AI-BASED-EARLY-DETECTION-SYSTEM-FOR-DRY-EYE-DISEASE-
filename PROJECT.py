...
try:
    ...
    arduino = None
    print("[WARNING] Arduino not found on COM6. Running in software-only mode.")
    ...
    while True:
        ...
        if len(rects) > 0:
            ...
            if face_locs:
                ...
                if firebase_enabled:
                    try: db.reference(f"user_history/{reg_name}").delete()
                    except: pass
...
total_blinks = 0
dry_eye_events = 0 # NEW: Tracks occurrences for the dashboard
COUNTER = 0
last_id_check = 0
session_start = None
csv_path = None
...
def upload_session_history(user, s_id, blinks, start_time, dry_events, temp, humidity):
    if firebase_enabled and user not in ["Scanning...", "Unregistered"] and start_time:
        duration_min = (time.time() - start_time) / 60
        if duration_min > 0.1: # Only save if the session was longer than 6 seconds
            try:
                history_ref = db.reference(f"user_history/{user}/session_{s_id}")
                history_ref.set({
                    "date": datetime.now().strftime("%Y-%m-%d"),
                    "time": datetime.now().strftime("%H:%M:%S"),
                    "duration_minutes": round(duration_min, 2),
                    "total_blinks": blinks,
                    "dry_eye_events": dry_events,
                    "avg_bpm": round(blinks / duration_min, 1),
                    "avg_temp": temp,          # <-- ADDED THIS
                    "avg_humidity": humidity   # <-- ADDED THIS
                })
                print(f"\n[FIREBASE] Saved Patient History for {user}")
            except Exception as e:
                print(f"[FIREBASE HISTORY ERROR] {e}")

def save_session_graph(user, s_id, bpm_data):
    if bpm_data and user not in ["Scanning...", "Unregistered"]:
        try:
            plt.figure(figsize=(10, 5))
            plt.plot(bpm_data, color='blue', marker='o', linestyle='-')
            plt.title(f"Blink Rate Analysis: {user} (Session {s_id})")
            ...
            safe_name = user.replace(" ", "_")
            graph_path = os.path.join(GRAPHS_DIR, f"{safe_name}_{s_id}.png")
            plt.savefig(graph_path)
            plt.close() # Silently closes the graph so it doesn't interrupt the camera
            print(f"[GRAPH] Saved {user}_{s_id}.png")
        except Exception as e:
                ...
                if face_encs:
                    ...
                    if True in matches:
                        ...
                        if match_name != current_user:
                            # -- UPLOAD HISTORY BEFORE SWITCHING USERS --
                            upload_session_history(current_user, session_id, total_blinks, session_start, dry_eye_events, current_temp, current_humidity)
                            save_session_graph(current_user, session_id, bpm_history)
    ...
    if session_start and current_user not in ["Scanning...", "Unregistered"]:
        ...
        if bpm >= 20: 
            ...
            if elapsed > 20 and (time.time() - last_boost_time > 20.0):
                dry_eye_events += 1 # Increase dry eye event count for dashboard
        ...
        if firebase_enabled and (time.time() - last_firebase_update > 3.0):
            try:
                # Pushing exactly to the branch expected by index.html
            ...
            if fresh_frame is not None:
                ...
                if face_locs_fresh:
                    ...
                    if True in matches:
                        ...
                        if firebase_enabled:
                            try: db.reference(f"user_history/{new_name}").delete()
                            except: pass
